/**
 * โค้ดต้นฉบับ Google Apps Script (Code.gs) สำหรับแสดงและดาวน์โหลดในหน้าตั้งค่า
 */
export const FULL_CODE_GS = `/**
 * =========================================================================
...
function doGet(e) {
  try {
    ...
    switch (action) {
      ...
      case 'getSettings':
        responseData = { status: 'success', data: getSettingsData(ss) };
        break;

      case 'getMenuBank':
        responseData = { status: 'success', data: getMenuBankData(ss) };
        break;

      case 'getDailyMenu':
        const monthFilter = params.month || '';
        const dateFilter = params.date || '';
        responseData = { status: 'success', data: getDailyMenuData(ss, monthFilter, dateFilter) };
        break;
...
function handleImageDelete(fileId) {
  if (!fileId) return { status: 'error', message: 'ไม่พบ File ID' };
  try {
